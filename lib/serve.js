...
const templateFileName = '_dgmp_template.html'
...
module.exports = function(port, llport, path) {
  ...
  path = resolve(path || '.')
  var server = connect()
  server.use(serveStatic(indexTemplatePath(path), { index: [templateFileName] }))
...
function indexTemplatePath(path) {
  ...
  const scriptPathUnderTemplatePath = npath.join(scriptPath, templateFileName)
  const nodeModuleUnderTemplatePath = npath.join(nodeModulePath, templateFileName)
  const pathUnderTemplate = npath.join(path, templateFileName)
  ...
  console.log(chalk.red('没有找到模版文件_dgmp_template.html'))
}
